...
class Texture {
    ...
    update(textureUnit = 0) {
        /* PATCHED_FOR_VIDEO_UPLOAD */
        // Always upload frames for HTMLVideoElement even when element reference unchanged
        const isVideoElement = (typeof HTMLVideoElement !== 'undefined') && (this.image instanceof HTMLVideoElement);
        const needsUpdate = isVideoElement ? true : !(this.image === this.store.image && !this.needsUpdate);
        ...
        if (this.image) {
            // Support HTMLVideoElement dimensions (videoWidth/videoHeight) as well as image.width/height
            if (this.image.videoWidth && this.image.videoHeight) {
